 ...
 * @since Total 1.6.0
 */
class STATUS_Fonts_Dropdown_Control extends WP_Customize_Control {
	public function render_content() {
		$this_val = $this->value(); ?>
	<label>
		<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
		<select <?php $this->link(); ?> style="width:100%;">
			<option value="" <?php if ( ! $this_val ) echo 'selected="selected"'; ?>><?php esc_html_e( 'Default', 'status' ); ?></option>
			<?php
			// Add custom fonts from child themes
			if ( function_exists( 'st_add_custom_fonts' ) ) {
				$fonts = st_add_custom_fonts();
				if ( $fonts && is_array( $fonts ) ) { ?>
					<optgroup label="<?php esc_html_e( 'Custom Fonts', 'status' ); ?>">
			...
			<optgroup label="<?php esc_html_e( 'Standard Fonts', 'status' ); ?>">
				<?php
				// Get standard font options
				$std_fonts = st_standard_fonts_array();
			...
			<optgroup label="<?php esc_html_e( 'Google Fonts', 'status' ); ?>">
				<?php
				// Get font options
				$google_fonts = st_google_fonts_array();
				// Loop through font options and add to select
 ...
 * @since 1.0.0
 */
function st_standard_fonts_array() {
	return apply_filters( 'st_standard_fonts_array', array(
